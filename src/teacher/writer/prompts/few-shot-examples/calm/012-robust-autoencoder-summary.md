...
The robustness ensures that when the generative model makes small prediction errors—which it inevitably will—these errors lead to graceful degradation in reconstruction quality rather than catastrophic failures. A slightly imperfect latent vector still decodes to mostly correct tokens, allowing the model to recover and continue generating coherently. With this robust autoencoder established, the authors turn to training the full continuous autoregressive system—combining the Transformer backbone with the energy-based generative head to predict sequences of continuous vectors.